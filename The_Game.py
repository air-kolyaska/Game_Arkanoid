# подключаем библиотеку с графикой и модули, которые отвечаюь за время и случайные числа
from tkinter import *
...
# делаем заголовок игры через свойства объекта title
game.title('Arkanoid')

# запрещаем изменять размеры окна, через свойство объекта resizable
game.resizable(0, 0)
...
canvas = Canvas(game, width=500, height=400, highlightthickness=0)

canvas.pack()  # говорим холсту, что у каждого видимого элемента будут свои отдельные координаты

game.update()  # обновляем окно с холстом



# Описываем класс Ball, который будет отвечать за шарик
class Ball:
    # конструктор — он вызывается в момент создания нового объекта
    def __init__(self, canvas, paddle, score, color):
    # на основе этого класса задаём параметры объекта, которые нам передают в скобках в момент создания
        self.canvas = canvas
        self.paddle = paddle
        self.score = score
    # параметр color здесь не задается, он используется для закрашивания круга, который мы будем рисовать

    # здесь используется новое свойство параметра self - id, в котором хранится внутреннее название шарика
    # и здесь же  командой create_oval() создаем круг радиусом 15 пикселей и закрашиваем его
        self.id = canvas.create_oval(10, 10, 25, 25, fill=color)

    # размещаем шарик в точку с координатами 245, 100
        self.canvas.move(self.id, 245, 100)

    # задаем список возможных направлений для старта и перемешиваем этот список
        directions = [-2, -1, 1, 2]
    ...
    # обрабатываем касание платформы, для этого получаем 4 координаты шарика в переменной
    # pos (левая верхняя и правая нижняя точки)
    ...
    # если координаты касания совпадают с координатами платформы
        if pos[2] >= paddle_pos[0] and pos[0] <= paddle_pos[2]:
            if pos[3] >= paddle_pos[1] and pos[3] <= paddle_pos[3]:
                ...
                self.score.hit()
                return True    # возвращаем метку о том, что мы успешно коснулись
    ...
    def draw(self):
        ...
        if pos[3] >= self.canvas_height:
            ...
            # и выводим сообщение и количество очков
...
class Score:
    ...
    # создаём надпись, которая показывает текущий счёт, делаем его нужно цвета и запоминаем внутреннее имя этой надписи
        self.id = canvas.create_text(450, 10, text=self.score, font=('Courier', 25), fill='blue')
...
#На этом блок описания классов заканчивается и переходим к написанию самой игры
#Нам нужно создать конкретные объекты шарика, платформы и счёта и сказать им, в каком порядке мы будем что делать.
